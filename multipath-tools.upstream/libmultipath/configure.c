...
setup_map (struct multipath * mpp, char * params, int params_size)
{
	...
	/*
	* Lets add the subsystem for statistics here
	* We'll have to check in the future if a value
	* statistics = 1 is enabled in multipath.conf
	* will be probably be off by default
	*/
	sysfs_set_stats(mpp);
